...
def check_file(filepath):
    # Ajuste Técnico: Verificação relativa à raiz do repositório apontando para /web
    full_path = os.path.join("web", filepath)
    if os.path.exists(full_path):
        print(f"[OK] Arquivo: {full_path}")
        return True
    else:
        # Tenta verificar se o arquivo está na raiz (fallback)
        if os.path.exists(filepath):
            print(f"[OK] Arquivo: {filepath}")
            return True
        print(f"\033[1;31m[ERRO] Arquivo Ausente: {full_path}\033[0m")
        return False
...
def main():
    ...
    print_header("2. INTEGRIDADE DOS ARQUIVOS (VaultMindOS Web)")
    critical_files = [
        "package.json",
        "next.config.ts",
        "src/app/layout.tsx",
    ...
    if all_systems_go:
        ...
        print("\033[1;31m🛑 STATUS VERMELHO: FALHA NA VERIFICACAO.\033[0m")
        sys.exit(1)
